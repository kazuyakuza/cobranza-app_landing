...
(2) Present Global Plan to user for approval. **Do NOT call `plan_exit`.**, use another way.
(3) Remain active as orchestrator for Steps 1–6. Execute global plan step by step using the `task` tool — never delegate all steps to one sub-agent.
